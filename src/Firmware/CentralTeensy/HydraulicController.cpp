...
// --- Update a ram's setpoint from a network command ---
void HydraulicController::updateSetpoint(const String& targetId, double value) {
    if (targetId == "ram_center") {
        _ramCenter.setpointPositionPercent = constrain(value, 0.0, 100.0);
    } else if (targetId == "ram_left") {
        _ramLeft.setpointPositionPercent = constrain(value, 0.0, 100.0);
    } else if (targetId == "ram_right") {
        _ramRight.setpointPositionPercent = constrain(value, 0.0, 100.0);
    }
}
